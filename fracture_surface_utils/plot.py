...
class Plotter:
    ...
    def plot_fitted_contour(self):
        ...
        sns.scatterplot(x=sliced_contour[x_coordinate]['Z_Coordinates_Raw'],
                        y=sliced_contour[x_coordinate]['Y_Coordinates_Raw'],
                        ax=axs, marker='^', label=f'Scanned Contour', )

        for segment in [item for item in piecewise_linear_fit[x_coordinate]['Segment_Index'].keys()]:
            sns.lineplot(x=piecewise_linear_fit[x_coordinate]['Segment_Index'][segment]['Z_Coordinates_Fit'],
                         y=piecewise_linear_fit[x_coordinate]['Segment_Index'][segment]['Y_Coordinates_Fit'],
                         ax=axs, label='Linear Fit', color='k')

        handles, labels = axs.get_legend_handles_labels()
        by_label = dict(zip(labels, handles))
        axs.legend(by_label.values(), by_label.keys(), bbox_to_anchor=(0, 1.02, 1, 0.2), loc="lower left",
    ...
    def plot_data_evaluation(self,  legend_off: bool = True):
        ...
        sns.scatterplot(x=sliced_contour[x_coordinate]['Z_Coordinates_Contour'],
                        y=sliced_contour[x_coordinate]['Y_Coordinates_Contour'],
                        ax=axs[0], marker='^', label=f'Contour', )
        ...
        for segment in [item for item in piecewise_linear_fit[x_coordinate]['Segment_Index'].keys()]:
            ...
            sns.lineplot(x=piecewise_linear_fit[x_coordinate]['Segment_Index'][segment]['Z_Coordinates_Fit'],
                         y=piecewise_linear_fit[x_coordinate]['Segment_Index'][segment]['Y_Coordinates_Fit'],
                         ax=axs[0], label='Linear Fit', color='k')

            array_lenght = len(piecewise_linear_fit[x_coordinate]['Segment_Index'][segment]['Z_Coordinates_Fit'])
            sns.lineplot(x=piecewise_linear_fit[x_coordinate]['Segment_Index'][segment]['Z_Coordinates_Fit'],
                         y=[piecewise_linear_fit[x_coordinate]['Segment_Index'][segment]['R_Squared']] * array_lenght,
                         ax=axs[1], color='k', label='Correlation-\n coeffizient')

            sns.lineplot(x=piecewise_linear_fit[x_coordinate]['Segment_Index'][segment]['Z_Coordinates_Fit'],
                         y=[modi_analysis[x_coordinate]['Fracture_Mode_Classification_Category']] * array_lenght,
                         ax=axs[3],
                         color=color_map[modi_analysis[x_coordinate]['Fracture_Mode_Classification_Category']],
                         label='Fracture Mode')

            sns.lineplot(x=piecewise_linear_fit[x_coordinate]['Segment_Index'][segment]['Z_Coordinates_Fit'],
                         y=[shearlip_analysis[x_coordinate]['Shearlip_Classification_Category'][
                                segment_idx]] * array_lenght,
                         color='k', ax=axs[2],
                         label='Shearlip')
        ...
        for i in range(4):
            ...
            by_label = dict(zip(labels, handles))
            if i == 0:
                axs[i].legend(by_label.values(), by_label.keys(), bbox_to_anchor=(0, 1.02, 1, 0.2),
                              loc="lower left", mode="expand", borderaxespad=0, ncol=1)
            else:
    ...
    def plot_xy_contour(self, contour_to_plot: str = 'Contour', limits_x_plot: list = None,
        ...
        sns.lineplot(x=sliced_contour_xy[self.analysis.z_coordinate_slice][f'X_Coordinates_{contour_id}'],
                        y=sliced_contour_xy[self.analysis.z_coordinate_slice][f'Y_Coordinates_{contour_id}'],
                        color='k', ax=ax1)

        sns.lineplot(x=sliced_contour_xy[self.analysis.z_coordinate_slice][f'X_Coordinates_{contour_id}'],
                        y=sliced_contour_xy[self.analysis.z_coordinate_slice][f'Kinking_Angle_{contour_id}[°]'],
                        color='k', ax=ax2)

        ax1.axis('equal',)
        ax1.set_ylim(sliced_contour_xy[self.analysis.z_coordinate_slice][f'Y_Coordinates_{contour_id}'].min()-20,
                     sliced_contour_xy[self.analysis.z_coordinate_slice][f'Y_Coordinates_{contour_id}'].max()+20,)
